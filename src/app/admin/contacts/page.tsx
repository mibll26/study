          ...
          return (
            <details key={c.id} className="border-2 border-ink bg-panel">
              ...
              <div className="grid gap-3 border-t border-line px-4 py-3 text-[13px] sm:grid-cols-2">
                ...
                <div>{c.formulationSlug && <div className="mb-1"><span className="text-muted-2">배합</span> <a href={`/formulate/${c.formulationSlug}`} target="_blank">/formulate/{c.formulationSlug} ↗</a></div>}<div className="text-muted-2">요청사항</div><p className="whitespace-pre-wrap">{c.message ?? "—"}</p>{c.adminNote && <p className="mt-2 text-muted"><span className="text-muted-2">메모</span> {c.adminNote}</p>}</div>
